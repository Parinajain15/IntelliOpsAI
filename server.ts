...
app.post('/api/recommendations/:id/status', (req, res) => {
  try {
    ...
    // Dynamic real impact simulation: if a "completed" state is reached, rebalance if relevant!
    if (status === 'Completed') {
      const anomalies = AnomalyDetectionService.detectAnomalies(db);
      const recs = RecommendationService.generateRecommendations(db, anomalies);
      const matchedRec = recs.find(r => r.id === id);

      if (matchedRec && matchedRec.category === 'Reassign overloaded employee tasks') {
        const matchEmp = matchedRec.riskSource ? matchedRec.riskSource.match(/Overloaded Employee - ([^(]+)/) : null;
        const employeeName = matchEmp ? matchEmp[1].trim() : null;
        const helperName = matchedRec.owner;

        if (employeeName && helperName) {
          const taskToMove = db.getTasks().find(t => t.employeeName === employeeName && t.status !== 'Completed');
          if (taskToMove) {
            const oldOwner = taskToMove.employeeName;
            taskToMove.employeeName = helperName;
            db.recalculateOperationalModels();

            db.addAuditLog(
              'RECOMMENDATION_EXECUTE',
              helperName,
              'Operations Manager',
              `Executed automatic load rebalance: Transferred task "${taskToMove.notes}" from ${oldOwner} to ${helperName} to resolve capacity limitations.`
            );
          }
        }
      } else if (matchedRec && matchedRec.category === 'Resolve SLA breach incidents') {
        const matchTask = id.match(/rec_sla_mit_(task_\d+)/);
        const taskId = matchTask ? matchTask[1] : null;
        if (taskId) {
          const dbTask = db.getTasks().find(t => t.id === taskId);
          if (dbTask) {
            ...
            db.addAuditLog(
              ...
              dbTask.employeeName,
              'Team Lead',
              `Manually prioritized and completed task "${dbTask.notes}" assigned to ${dbTask.employeeName} under recommendation plan ${id}.`
            );
          }
        }
      } else {
        db.addAuditLog(
          ...
          'Operations Team',
          'Operations Manager',
          `Authorized general operational stabilization under plan ${id}: "${matchedRec?.title}".`
        );
      }
    }
